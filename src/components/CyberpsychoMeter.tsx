...
/* ── Random helpers (stable per render via useMemo) ── */
...
const WIDGETS: Widget[] = [
  { label: 'HEART RATE', icon: Activity, color: '#FF2D2D', base: 182, unit: 'BPM', variance: 4, int: true, x: 4, y: 14, dur: 9, delay: 0 },
  { label: 'NEURAL LOAD', icon: Cpu, color: '#00F0FF', base: 79, unit: '%', variance: 6, int: true, x: 82, y: 10, dur: 11, delay: 1.5 },
  { label: 'MEME EXPOSURE', icon: Radio, color: '#FF00A8', base: 0, unit: '', variance: 0, int: false, x: 6, y: 72, dur: 10, delay: 0.8 },
  { label: 'NETWORK STATUS', icon: Wifi, color: '#39FF14', base: 0, unit: '', variance: 0, int: false, x: 78, y: 76, dur: 12, delay: 2 },
  { label: 'BLOCKCHAIN LATENCY', icon: ShieldCheck, color: '#FFE600', base: 12, unit: 'ms', variance: 3, int: true, x: 40, y: 4, dur: 8, delay: 0.3 },
];
...
const STATUS_INDICATORS = [
  { text: 'ONLINE', x: 14, y: 8 },
  { text: 'SYNC', x: 88, y: 18 },
  { text: 'LOCKED', x: 10, y: 44 },
  { text: 'ACTIVE', x: 90, y: 48 },
  { text: 'TRACKING', x: 16, y: 88 },
  { text: 'VERIFYING', x: 84, y: 90 },
  { text: 'MONITORING', x: 50, y: 94 },
  { text: 'CONNECTED', x: 92, y: 64 },
];
...
export default function CyberpsychoMeter() {
  ...
  // Intensity 0..1 derived from meter — drives HUD reactivity.
  const intensity = level / 100;

  // Stable random geometry for the network + waveform.
  ...
  // Live diagnostic values — smoothly drift toward new targets.
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setVitals((prev) =>
        prev.map((v, i) => {
          const w = WIDGETS[i];
          if (w.label === 'MEME EXPOSURE' || w.label === 'NETWORK STATUS') return v;
  ...
  // Waveform canvas — amplitude & frequency scale with meter level.
    ...
    <section id="meter" ref={sectionRef} className="relative overflow-hidden px-5 py-24">
      ...
      <div className="relative z-10 mx-auto max-w-3xl">
        ...
        <div className="relative reveal-pop">
          ...
          <div className="relative clip-cyber scan-card border border-cyber-magenta/40 bg-cyber-panel/70 p-8 backdrop-blur-sm"
            ...
            <div className="mb-6 flex items-center justify-between font-mono text-[10px] tracking-[0.3em] text-gray-500">
              ...
              <span className="animate-flicker text-cyber-cyan/70">REC ●</span>
            </div>
      ...
      {WIDGETS.map((w, i) => {
        const val = w.label === 'MEME EXPOSURE' ? MEME_LEVELS[memeIdx]
          : w.label === 'NETWORK STATUS' ? NET_STATES[netIdx]
      ...
      {STATUS_INDICATORS.map((s, i) => (
        ...
        >
          <span className="hud-status-dot" style={{ animationDelay: `${i * 0.3}s` }} />
          <span>{s.text}</span>
        </div>
      ))}

      <HudStyles intensity={intensity} />
    </section>
  );
}
...
function HudBackground({
  ...
  const scans = useMemo(() => Array.from({ length: 5 }, () => ({
    dir: pick(['h', 'v', 'd'] as const), pos: rnd(8, 92), dur: rnd(7, 13), delay: rnd(0, 8),
  })), []);
...
function HudStyles({ intensity }: { intensity: number }) {
  return (
    <style>{`
...
.hud-radar-beam {
  ...
  animation: hudRadarSpin ${radarDurFor(intensity)} linear infinite;
...
.hud-status-dot {
  ...
  animation: hudStatusBlink 1.6s steps(1) infinite;
}

@media (prefers-reduced-motion: reduce) {
  .hud-aurora, .hud-grid, .hud-radar-beam, .hud-particle, .hud-scan,
  .hud-widget, .hud-network circle { animation: none !important; }
}
`}</style>
  );
}

function radarDurFor(intensity: number) {
  return `${(6 - intensity * 2).toFixed(2)}s`;
}
